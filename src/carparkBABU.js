...
const floorContainsCar = (floor) => floor.includes(CAR);
const isAnArray = (building) => Array.isArray(building);
const isMyCarParkedOutsideTheParking = (building) => !building.some(floorContainsCar);
...
const carFloorIndex = (building) => building.findIndex(floorContainsCar);
const floorSize = (building) => building[0].length; // !! WIP flat grocery parking
const whereIsMyCar = (building) => building[carFloorIndex(building)].indexOf(CAR); // !! WIP flat grocery parking
